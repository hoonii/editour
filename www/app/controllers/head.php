...
class Head extends Widget {
	function index($data=FALSE) {
		...
		if(strrpos($this->input->server('REQUEST_URI'), "admin") == false){
			...
			$this->load->view('_admin_head', $head);
		}
	}
	...
	function admin($admin=FALSE) {
		...
		return array(
			'skin' => $this->load->view('admin/head', $admin, TRUE),
			'body' => ''
		);
    }
	...
	function popup($popup=FALSE) {
		define('ADM_PATH', '/admin');
		$popup['current'] = ADM_PATH.$this->uri->slash_segment(2, 'both');
		
		return array(
			'skin' => $this->load->view('popup/head', $popup, TRUE),
			'body' => 'body_gray2'
		);
    }
	
	
	function temp($temp=FALSE) {
		define('ADM_PATH', '/admin');
		$temp['current'] = ADM_PATH.$this->uri->slash_segment(2, 'both');
		
		return array(
			'skin' => $this->load->view('temp/head', $temp, TRUE),
			'body' => 'body_gray2'
		);
    }
	...
	function emptySkin() {
		return array(
			'skin' => $this->load->view('empty/head', null, TRUE),
			'body' => ''
		);
    }
}