...
class Login extends CI_Controller {
	...
	function in() {
		...
		if ($this->form_validation->run() !== FALSE) {
			...
			$mb = $this->Basic_model->get_member($this->input->post('mb_id'), 'mb_id, mb_password, mb_email, mb_level, mb_leave_date, mb_email_certify');
...
// 			$this->session->set_userdata('ss_mb_id', $mb['mb_id']);
			$this->session->set_userdata('ss_mb_email', $mb['mb_email']);
			...
			if($mb['mb_level'] >= 10){
				goto_url($this->config->item('base_url') . "/adm");
			} else {
