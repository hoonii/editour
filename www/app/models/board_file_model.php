...
class Board_file_model extends CI_Model {
	...
	function file_insert($bo_table, $wr_id, $values, $editor=0) {
		...
		$this->db->query('insert into board_file ( bo_table, wr_id, bf_editor, bf_no, bf_source, bf_file, bf_download, bf_filesize, bf_width, bf_height, bf_type, bf_datetime ) values '.$values);
	...
	function get_files($bo_table, $wr_id, $field='*', $editor=0) {
		...
		$qry = $this->db->get_where('board_file', $sql);
	...
	function get_file($bo_table, $wr_id, $bf_no) {
		...
		$qry = $this->db->get_where('board_file', array(
			'bo_table' => $bo_table,
			'wr_id' => $wr_id,
			'bf_editor' => 0,
			'bf_no' => $bf_no
	...
	function file_down_update($bo_table, $wr_id, $bf_no) {
		...
		$this->db->update('board_file', null, array(
			'bo_table' => $bo_table,
			'wr_id' => $wr_id,
			'bf_editor' => 0,
			'bf_no' => $bf_no
