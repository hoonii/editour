...
class Board_comt_model extends CI_Model {
	...
	function same_comment($bo_table, $wr_id, $commend_id) {
		...
		return $this->db->order_by('co_id', 'desc')->get('comment', 1)->row_array();
	}
	...
	function get_max_num($bo_table, $wr_id) {
		...
		$row = $this->db->get_where('comment', array(
			'bo_table' => $bo_table,
			'wr_id' => $wr_id
		))->row_array();
	...
	function comment_insert($bo_table, $wr_id, $tmp_num, $tmp_reply, $ca_code, $mb) {
		...
		$this->db->update('board', null, array('bo_table' => $bo_table));
	...
	function comment_update($bo_table, $wr_id, $comment_id) {
		...
		$this->db->update('comment', $sql, array(
			'bo_table' => $bo_table,
			'wr_id' => $wr_id,
			'co_id' => $comment_id
		));
