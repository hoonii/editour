...
class Board_model extends CI_Model {
	...
	function list_result($bo_table, $spt, $sca, $sst, $sod, $sfl, $stx, $limit, $offset, $wr_field) {
		...
		return $this->db->get('write', $limit, $offset)->result_array();
	}
	...
	function list_count($bo_table, $spt, $sca, $sfl, $stx) {
		...
		return $this->db->count_all_results('write');
	}
	...
	function hit_update($bo_table, $wr_id) {
		...
		$this->db->update('write', null, array('bo_table' => $bo_table, 'wr_id' => $wr_id));
	}
	...
	function prev_next_link($bo_table, $wr_num, $wr_reply, $sca, $sfl, $stx) {
		...
		$prev = $this->db->get('write', 1)->row_array();
		...
		$this->_get_search_cache($sca, $sfl, $stx);
		$this->db->order_by('wr_num desc, wr_reply desc');
		$next = $this->db->get('write', 1)->row_array();
	...
	function is_reply($bo_table, $wr_id, $wr_num, $wr_reply) {
		...
		return ($this->db->count_all_results('write') > 0) ? TRUE : FALSE;
	}
	...
	function get_reply_step($bo_table, $wr_num, $bo_reply_order, $wr_reply) {
		...
		$row = $this->db->get('write')->row_array();
	...
	function get_min_num($bo_table) {
	    ...
	    $row = $this->db->get_where('write', array(
			'bo_table' => $bo_table
		))->row_array();
	...
	function write_insert($bo_table, $wr_content, $wr_num, $wr_reply, $mb, $bo_notice) {
	    ...
	    $this->db->update('board', $sql, array('bo_table' => $bo_table));
	...
	function write_update($bo_table, $wr_content, $wr_id, $mb, $bo_notice) {
		...
		$this->db->update('write', $sql, array('bo_table' => $bo_table, 'wr_id' => $wr_id));
	
	    // 분류가 수정되는 경우 해당되는 댓글의 분류명도 모두 수정함
	    // 댓글의 분류를 수정하지 않으면 검색이 제대로 되지 않는다.
	    if ($ca_code) 
	    	$this->db->update('comment', array('ca_code' => $ca_code), array('bo_table' => $bo_table, 'wr_id' => $wr_id));
	    
	    // 공지사항
	    if (IS_ADMIN)
	    	$this->db->update('board', array('bo_notice' => trim($bo_notice)), array('bo_table' => $bo_table));
	}
	...
	function write_delete($bo_table, $wr_ids, $bo_notice, $bo_min_wr_num, $bo_extra) {
		...
		$result = $this->db->get('board_file')->result_array();
		...
		// 게시물, 파일테이블 삭제
		$this->db->where('bo_table', $bo_table)->where_in('wr_id', $wr_ids);
		$this->db->delete(array('write', 'board_file'));
		...
		if ($bo_extra) {
			...
			$this->db->delete('extra_'.$bo_table);
		}
	}

	function content_update($bo_table, $wr_id, $content) {
        $this->db->update('write', array(
            'wr_content' => $content
	...
	function get_extra($bo_table, $wr_id) {
		if (!is_array($wr_id)) {
			return $this->db->get_where('extra_'.$bo_table, array(
				'wr_id' => $wr_id
			))->row_array();
		}
		else {
			$this->db->where_in('wr_id', $wr_id);
			return $this->db->get('extra_'.$bo_table)->result_array();
		}
	}

	function extra_update($w, $bo_table, $wr_id, $sql) {
		if ($w == 'u')
			$this->db->update('extra_'.$bo_table, $sql, array('wr_id' => $wr_id));
		else {
			$sql['wr_id'] = $wr_id;
			$this->db->insert('extra_'.$bo_table, $sql);
		}
	}
}
?>