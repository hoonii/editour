...
class Member_memo_model extends CI_Model {
	...
	function list_result($flag, $mb_id, $limit, $offset) {
		...
		$this->db->join('member b', 'a.me_mb_id = b.mb_id');
		$this->db->order_by('me_no', 'desc');
		$qry = $this->db->get_where('memo a', array('a.me_flag' => $flag, 'a.mb_id' => $mb_id), $limit, $offset);
		return $qry->result_array();
	}

	function get_memo($me_no, $flag, $mb_id) {
		$this->db->select('me_no, me_mb_id, me_content, me_datetime, me_check');
		$query = $this->db->get_where('memo', array(
			'me_no' => $me_no,
			'me_flag' => $flag,
			'mb_id' => $mb_id
		));
	...
	function insert($mb_id, $me_mb_id, $me_content) {
		...
		$this->db->update('member', array('mb_memo_call' => $mb_id), array('mb_id' => $me_mb_id));
	}
	...
	function call_delete($mb_id) {
		$this->db->update('member', array('mb_memo_call' => ''), array('mb_id' => $mb_id));
	}
	
	function memo_count($mb_id, $cnt='1') {
		$this->db->set('mb_memo_cnt', 'mb_memo_cnt - '.$cnt, FALSE);
		$this->db->update('member', null, array('mb_id' => $mb_id));
	}
}
?>