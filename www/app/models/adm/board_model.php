...
class Board_model extends CI_Model {
	...
	function is_group() {
		$row = $this->db->count_all_results('board_group');
	...
	function update($is_notice='') {
		...
		$sql['bo_count_write'] = $this->db->count_all_results('write');

		$this->db->where('bo_table', $this->input->post('bo_table'));
		$sql['bo_count_comment'] = $this->db->count_all_results('comment');
		...
		$this->db->update('board', $sql, array('bo_table' => $this->input->post('bo_table')));
	}
	...
	function group_update() {
		...
		$this->db->update('board', $sql, array('gr_id' => $this->input->post('gr_id')));
	}
	...
	function proc_count() {
		$this->db->query("
			UPDATE write a JOIN (
				SELECT bo_table, wr_id, COUNT(co_id) AS cnt FROM comment WHERE bo_table = '".$this->input->post('bo_table')."' GROUP BY wr_id
	...
	function delete($bo_tables, $ca_types) {
		...
		$this->db->delete(array('board', 'board_file', 'write'));
