...
class Popup_model extends CI_Model {
	...
	function list_result($sst, $sod, $sfl, $stx, $limit, $offset) {
		...
		$qry = $this->db->get('popup', $limit, $offset);
	...
	function get_popup($pu_id, $fields='*') {
		...
		return $this->db->select($fields)->get_where('popup', array('pu_id' => $pu_id))->row_array();
	}
	...
	function record($w='') {
		...
		if ($w == '') {
			...
			$this->db->insert('popup', $sql);
			return $this->db->insert_id();
		}
		else {
			$this->db->update('popup', $sql, array('pu_id' => $this->input->post('pu_id')));
	...
	function list_update($pu_ids, $pu_names, $pu_uses) {
		...
		$this->db->update_batch('popup', $batch, 'pu_id');
	}
