...
class Boardgroup_model extends CI_Model {
	...
	function list_result($sst, $sod, $sfl, $stx, $limit, $offset) {
		...
		$result['total_cnt'] = $this->db->count_all_results('board_group a');

		$this->db->join('board b', 'a.gr_id = b.gr_id', 'left');
		$this->db->select('a.gr_id,a.gr_subject,a.gr_admin, count(b.bo_table) as bo_cnt');
		$this->db->order_by($sst, $sod);
		$this->db->group_by('gr_id');
		$qry = $this->db->get('board_group a', $limit, $offset);
	...
	function get_group($gr_id) {
		...
		$query = $this->db->get_where('board_group', array('gr_id' => $gr_id));
	...
	function insert() {
		...
		$this->db->insert('board_group', $sql);
	}
	...
	function update() {
		...
		$this->db->update('board_group', $sql, array('gr_id' => $this->input->post('gr_id')));
	}
	...
	function list_update($gr_id, $gr_subject, $gr_admin) {
		...
		$this->db->update('board_group', $sql, array('gr_id' => $gr_id));
	}

	function delete() {
		$this->db->where('gr_id', $this->input->post('gr_id', TRUE));
		$row = $this->db->count_all_results('board');
		if ($row > 0)
			return FALSE;
		else
			return $this->db->delete('board_group', array('gr_id' => $this->input->post('gr_id', TRUE)));
	}
}
?>