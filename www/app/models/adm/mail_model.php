...
class Mail_model extends CI_Model {
	...
	function list_result($limit, $offset) {
		$result['total_cnt'] = $this->db->count_all_results('mail');

		$this->db->select('ma_id,ma_subject,ma_content,ma_time');
		$this->db->order_by('ma_id', 'desc');
		$qry = $this->db->get('mail', $limit, $offset);
	...
	function get_mail($ma_id, $fields='*') {
		...
		$qry = $this->db->get_where('mail', array('ma_id' => $ma_id));
	...
	function update() {
		...
		$this->db->update('mail', $sql, array('ma_id' => $this->input->post('ma_id')));
	}

	function delete() {
		$this->db->delete('mail', array('ma_id' => $this->input->post('ma_id'))); 
	}
	...
	function select_list() {
		...
		$result['select_cnt'] = $this->db->count_all_results('member');
	...
	function option_update() {
		...
		$this->db->update('mail', array('ma_last_option' => $ma_last_option), array('ma_id' => $this->input->post('ma_id')));
	}
}
?>