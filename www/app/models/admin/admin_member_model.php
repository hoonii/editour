...
class Admin_Member_model extends CI_Model {
	...
	function list_result($sst, $sod, $sfl, $stx, $limit, $offset) {
		...
		$result['total_cnt'] = $this->db->count_all_results('member');

		$this->db->select('mb_id,mb_name,mb_email,mb_nick,mb_level,mb_point,mb_mailling,mb_open,mb_email_certify,mb_today_login,mb_leave_date,mb_open_date');
		$this->db->order_by($sst, $sod);
		$qry = $this->db->get('member', $limit, $offset);
		$result['qry'] = $qry->result_array();

		$this->db->where('mb_leave_date <>', '');
		$result['leave_cnt'] = $this->db->count_all_results('member');
	...
	function update() {
		...
		$this->db->update('member', $sql, array('mb_id' => $this->input->post('mb_id')));
	}
	...
	function delete($mb_ids) {
		...
		// 포인트 & 쪽지 삭제
		$this->db->where_in('mb_id', $mb_ids);
		$this->db->delete(array('point','memo')); 

		// 그룹관리자인 경우 그룹관리자를 공백으로
		$this->db->where_in('gr_admin', $mb_ids);
		$this->db->update('board_group', array('gr_admin' => ''));

		// 게시판관리자인 경우 게시판관리자를 공백으로
		$this->db->where_in('bo_admin', $mb_ids);
		$this->db->update('board', array('bo_admin' => ''));
	}
	 ...
	 */
	function update_member_point($point, $mb_email){
		$sql  = " UPDATE member SET ";
		$sql .= "    mb_point = mb_point - '" .$point. "'";
