...
class Board_mvcp_model extends CI_Model {
	...
	function list_move_copy($bo_table, $mb_id) {
		...
		return $this->db->get('board a')->result_array();
	}
	...
	function get_min_num($bo_table) {
	    ...
	    $row = $this->db->get_where('write', array('bo_table' => $bo_table))->row_array();
	...
	function get_dist_num($bo_table, $wr_ids) {
		...
		$result = $this->db->get('write')->result_array();
	...
	function get_write_file($bo_table, $wr_id) {
    	...
    	return $this->db->get_where('board_file', array(
    		'bo_table' => $bo_table,
    		'wr_id' => $wr_id
		))->result_array();
	}
	...
	function bo_count_update($bo_table, $count_write, $count_comment, $min_wr_num, $op) {
		...
		$this->db->update('board', array('bo_min_wr_num' => $min_wr_num), array('bo_table' => $bo_table)); 
	}
	...
	function write_delete($bo_table, $wr_ids) {
		...
		// 게시글, 파일테이블, 댓글 삭제
		$this->db->where('bo_table', $bo_table)->where_in('wr_id', $wr_ids);
		$this->db->delete(array('write', 'board_file', 'comment'));
	}
