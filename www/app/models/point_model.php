...
class Point_model extends CI_Model {
	...
	function insert($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='') {
		...
		$query = $this->db->get_where('point', array('mb_id'=>$mb_id));
		$row = $query->row_array();
		$sum_point = $row['sum_po_point'];

		// 포인트 UPDATE
		$this->db->update('member', array('mb_point'=>$sum_point), array('mb_id' => $mb_id));
	...
	function delete($mb_id, $rel_table, $rel_id, $rel_action) {
		...
		if ($rel_table || $rel_id || $rel_action) {
			...
			// 포인트 UPDATE
			$result = $this->db->update('member', array('mb_point'=>$sum_point), array('mb_id' => $mb_id));
		}
