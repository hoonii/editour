...
class Search_model extends CI_Model {
	...
	function search_board($mb_level, $stx) {
		// 인기검색어
        $this->db->simple_query(" insert into popular set pp_word = '".$stx."', pp_date = '".TIME_YMD."', pp_ip = '".$this->input->server('REMOTE_ADDR')."' ");

		$this->db->select('gr_id, bo_table, bo_read_level');
		$this->db->order_by('bo_order_search, gr_id, bo_table');
		$qry = $this->db->get_where('board', array(
			'bo_use_search' => 1,
	...
	function list_result($type, $stx, $limit, $offset, $boards) {
		...
		if ($type == 'write') {
			...
			$result['total_count'] = $this->db->count_all_results('write');
			
			
			$this->db->select('bo_table, wr_id, wr_subject, wr_content, wr_name, wr_datetime');
			$this->db->order_by('wr_datetime desc');
			$result['qry'] = $this->db->get('write', $limit, $offset)->result_array();
			...
			$result['total_count'] = $this->db->count_all_results('comment');

			$this->db->select('bo_table, wr_id, co_id, co_content, co_name, co_datetime');
			$this->db->order_by('co_datetime desc');
			$result['qry'] = $this->db->get('comment', $limit, $offset)->result_array();
